...
public interface LabOrderRepository extends JpaRepository<LabOrder, Long> {
        ...
        /**
         * Counts orders that are PENDING/IN_PROGRESS and contain at least one test
         * that is NOT marked as 'skipWorklist'.
         */
        @Query("""
                        SELECT COUNT(DISTINCT o)
                        FROM LabOrder o
                        ...
                        WHERE o.status NOT IN ('COMPLETED', 'CANCELLED')
                        AND (td.skipWorklist IS NULL OR td.skipWorklist = false)
                        """)
        long countActualLabPending();

        /**
         * Counts orders that are COMPLETED today and contain at least one test
         * that was NOT a 'skipWorklist' test.
         */
        @Query("""
                        SELECT COUNT(DISTINCT o)
                        FROM LabOrder o
                        ...
                        AND (td.skipWorklist IS NULL OR td.skipWorklist = false)
                        """)
        long countActualLabCompletedInRange(LocalDateTime start, LocalDateTime end);
        ...
        /**
         * Normalizes stale in-progress locks (orders opened for review but not
         * started).
         *
